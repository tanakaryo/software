...
public class PubSubBigtableWritePipeline {
        ...
        p.apply("Read PubSub",
                ...
                .apply(ParDo.of(new DoFn<StudentDto,Mutation>() {
                    ...
                    public void processElement(@Element StudentDto dto, OutputReceiver<Mutation> out) {
                        ...
                        long timestamp = System.currentTimeMillis();
                        String rowKey = dto.getStudentId() + "#" + dto.getSchoolType();
                        Put row = new Put(Bytes.toBytes(rowKey));

                        // StudentId
                        row.addColumn(Bytes.toBytes("student"), Bytes.toBytes("student_id"), timestamp, Bytes.toBytes(dto.getStudentId()));
                        // StudentName
                        row.addColumn(Bytes.toBytes("student"), Bytes.toBytes("student_name"), timestamp, Bytes.toBytes(dto.getStudentName()));
                        // ClassGrade
                        row.addColumn(Bytes.toBytes("class"), Bytes.toBytes("class_grade"), timestamp, Bytes.toBytes(dto.getClassGrade()));
                        // ClassNumber
                        row.addColumn(Bytes.toBytes("class"), Bytes.toBytes("class_number"), timestamp, Bytes.toBytes(dto.getClassNumber()));
                        // SchoolType
                        row.addColumn(Bytes.toBytes("school"), Bytes.toBytes("school_type"), timestamp, Bytes.toBytes(dto.getSchoolType()));
                        out.output(row);
                    }
                }))
